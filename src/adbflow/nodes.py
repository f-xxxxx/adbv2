...
class SwipeNode(BaseNode):
    ...
    def run(self, inputs: dict[str, Any], ctx: NodeContext) -> Any:
        ...
        if None not in (x1, y1, x2, y2):
            ...
            sx1, sy1, sx2, sy2 = _direction_swipe_coords(ctx.adb, device_id, direction)

        direction_label = "下滑" if direction == "down" else "上滑"
        ctx.adb.swipe(device_id, sx1, sy1, sx2, sy2, duration_ms=duration_ms)
        ctx.log(f"[滑动节点] 设备={device_id}，方向={direction_label}，({sx1},{sy1})->({sx2},{sy2})")
        return {
            **payload,
            "last_action": "swipe",
            "swipe": {"x1": sx1, "y1": sy1, "x2": sx2, "y2": sy2, "duration_ms": duration_ms},
        }
...
class ScreenshotNode(BaseNode):
    ...
    def run(self, inputs: dict[str, Any], ctx: NodeContext) -> Any:
        ...
        swipe_duration_ms = self._as_int(inputs.get("swipe_duration_ms"), 350)
        ...
        ctx.log(
            f"[截图节点] 设备={device_id}，张数={capture_count}，滚动={'是' if scroll else '否'}，手机目录={remote_dir}"
        )
        return {
            **payload,
            "device_id": device_id,
